...
def IdPayCallBack(request):
    ...
    try:
        ...
        if order.isPaid:
            return HttpResponseRedirect(f"{BASE_URL}/#/payresult/{trackIdpay}/{order_id}/{transId}?db=Null")
    except:
        ...
        if(str(callBackPayStatus) == '10'):
            try:
                ...
                return HttpResponseRedirect(f"{BASE_URL}/#/payresult/{trackIdpay}/{order_id}/{transId}?db={dbStatus}")
            else:
