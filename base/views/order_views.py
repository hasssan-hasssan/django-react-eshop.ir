        ...
        if order.isPaid:
            ...
            try:
                ...
                if(str(response.status_code).startswith('2')):
                    ...
                    return Response({'detail':f'Unknown Error status code: {response.status_code}'}, status=status.HTTP_400_BAD_REQUEST)
                
                elif(str(response.status_code).startswith('5')):
                    return Response({'detail':f'Unknown Error status code: {response.status_code}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
